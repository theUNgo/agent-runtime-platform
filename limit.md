...
- workflow skill 的 `when` 应支持轻量函数式条件，用于空值判断、集合/文本包含、长度判断、时间/日期比较和显式类型转换
- workflow skill 应支持函数嵌套与集合函数，避免为了简单判断再引入更重 DSL
- workflow skill 允许显式声明能力文档读取步骤，并默认遵循“先读 summary，再按条件决定是否读 detail”的动态加载顺序
...
- 涉及 Git 提交、推送、发布、PR 整理时，优先按 skill 化流程处理，而不是临时手写零散步骤
- 当前环境存在可用 Git / GitHub skill 时，默认优先使用对应 skill 的工作方式来收口变更、检查范围、提交和同步远端
- Git 提交前应先确认变更范围、敏感信息清理状态和构建结果，再执行提交或推送
- 若由于网络、权限或远端历史分叉无法直接 `git push`，允许采用可审计的替代同步方案，但仍要保持分支和远端状态可追踪
